...
def _create_argsparser():
    ...
    group.add_argument(
        '-k', '--stack-size', required=True,
        help='size of the stack for the app entry point thread, including the Thread Control Block; format: [\\d]+ or "0x[0-9A-Fa-f]+"',
        dest='stacklen', metavar='SIZE'
    )
    group.add_argument(
        '-c', '--config-struct', required=False, default='app_config',
        help='name of the app configuration struct',
        dest='appcfg', metavar='VAR_NAME'
    )
    group.add_argument(
        '-y', '--thread-entry', required=False, default='app_thread',
        help='name of the app thread entry point function',
        dest='appthreadf', metavar='FUNC_NAME'
    )
    ...
    group.add_argument(
        ...
        dest='ldoself', metavar='SYS_ELF_PATH'
    )
    group.add_argument(
        '-a', '--app-elf', required=True,
        help='path of the app ELF file',
        dest='ldobjelf', metavar='APP_ELF_PATH'
    )
    group.add_argument(
        '-x', '--app-hex', required=True,
        help='path of the app IHEX file',
        dest='hexpath', metavar='APP_IHEX_PATH'
    )
    group.add_argument(
        '-l', '--ld-cmd', required=False, default='ld',
        help='linker command name',
        dest='ldexe', metavar='LD_CMD'
    )
    group.add_argument(
        '-g', '--ld-gcc', required=False, action='store_true',
...
def _main():
    ...
    try:
        transport.open()
        bootloader.stop()
        bootloader.load(appname = str(args.appname),
                        hexpath = str(args.hexpath),
                        stacklen = autoint(args.stacklen),
                        ldexe = str(args.ldexe),
                        ldgcc = bool(args.ldgcc),
                        ldscript = str(args.ldscript),
                        ldoself = str(args.ldoself),
                        ldmap = str(args.ldmap),
                        ldobjelf = str(args.ldobjelf),
                        ldobjs = [ str(obj) for obj in args.ldobjs ])
        transport.close()

...
void Middleware::initialize(void *mgmt_boot_stackp, size_t mgmt_boot_stacklen, Thread::Priority mgmt_boot_priority) {
	...
	mgmt_boot_threadp = Thread::create_static(mgmt_boot_stackp, mgmt_boot_stacklen, mgmt_boot_priority, mgmt_threadf,
			NULL, "R2P_MGMT");
...
bool Middleware::advertise(LocalPublisher &pub, const char *namep, const Time &publish_timeout, size_t type_size) {
...
bool Middleware::advertise(RemotePublisher &pub, const char *namep, const Time &publish_timeout, size_t type_size) {
...
bool Middleware::subscribe(LocalSubscriber &sub, const char *namep, Message msgpool_buf[], size_t msgpool_buflen,
		size_t type_size) {
...
bool Middleware::subscribe(RemoteSubscriber &sub, const char *namep, Message msgpool_buf[], size_t msgpool_buflen,
		size_t type_size) {
		...
		if (node.spin(Time::ms(MGMT_TIMEOUT_MS))) {
			...
			while (sub.fetch(msgp, deadline)) {
				...
				case MgmtMsg::CMD_ADVERTISE: {
					...
					sub.release(*msgp);
					// NOTE: has_local_subscribers() instead of is_awaiting_advertisement() so a remote publisher is created on every pub module
...
//						transportp->advertise_cb(*topicp);
				...
				case MgmtMsg::CMD_GET_NETWORK_STATE: {
					...
					strncpy(msgp->module.name, get_module_name(), NamingTraits<Middleware>::MAX_LENGTH);
					SysLock::acquire();
...
Middleware::Middleware(const char *module_namep, const char *bootloader_namep) :
		module_namep(module_namep), lists_lock(false), mgmt_boot_stackp(NULL), mgmt_boot_stacklen(0), mgmt_boot_priority(
				Thread::LOWEST), mgmt_topic("R2P", sizeof(MgmtMsg)), mgmt_boot_threadp(NULL), boot_topic(
				bootloader_namep, sizeof(BootMsg)), topic_iter(topics.end()), stopped(false), num_running_nodes(0) {
